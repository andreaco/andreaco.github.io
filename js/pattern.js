class Pattern {
    ...
    matingAlternate(matePattern, index = 1) { 
        if(this.steps != matePattern.steps) { console.error('Mating patterns have different size') }
    ...
    */
   mating(patternB){
    return this.matingAlternate(patternB);
   }
}
...
class Population {
    ...
    selection(){
        for(let i=0; i<this._pool.length; i++){
            this.pool[i]._score = this.fitness(this.pool[i]);
        }
        console.log(this.pool[1])
        this._pool.sort((a,b) => (b.getScore() - a.getScore()))
    
        let trimmedOffspring = Math.floor(0.9*this.numberOfElements);
